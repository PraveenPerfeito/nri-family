...
This doc traces each section of the Phase 1 master prompt to where it is implemented. **Deviation** marks anything intentionally different, with the reason. The later UI V2 restyle is traced separately in [ui-v2.md](ui-v2.md).
...
| 12 | Hero copy, CTAs, microcopy, labelled "Family Office" preview | `HomeHero` + `CommandCenter` (labelled "Sample data · Concept preview") |
| 13 | Problem headline, copy, five cards | `DistanceStory`. **Deviation (UI V2):** the five questions are an editorial list rather than cards. |
| 14 | Solution headline, five cards, "See How It Works" | `EcosystemRadial` (the five areas around the family office) |
| 15 | Five service categories with bullets and CTAs; careful legal and medical wording | Home: `ServicesShowcase`; `/services`: `ServiceCard` × 5; scope notes from [services.ts](../../src/config/services.ts) |
| 16 | "Distance shouldn't mean uncertainty", six trust cards, 7-step timeline | Home: `EvidenceTimeline` (timestamped), `InspectionReport`, `TrustRecord`; `/trust`: `TrustRecord`, `FlowChain` |
| 17 | Privacy section, four cards, "Explore Trust & Security" | `PrivacyPrinciples` + `PrivacyAccess` diagram + CTA |
| 18 | Future family office, ecosystem, Available now vs Coming | `FutureOffice` ("Available today" vs "Coming to the platform", from `roadmap`) |
| 19 | Five-step how it works, "Create Your Family Office" | Home: `Journey` (six stages in UI V2) + CTA (→ `/get-started` until accounts exist); `/how-it-works`: `FlowChain` + `Timeline` |
| 20 | Dashboard preview with the specified figures | `WorkspaceDashboard` (3 / 2 / 1; activity; 92 / 96 / 88; approval card), labelled |
| 21 | Sell/rent section, three visibility levels, "List My Property" → `/get-started` | `PropertyControl` (visibility ladder + sample listing) |
...
| 37 | Footer columns, tagline, "© 2026 [Brand Name]", no invented company info | `Footer`: brand and optional contact details from site.ts; the legal name stays hidden until provided |
...
| 61–62 | 10-second understanding: trust, control, transparency, privacy, local execution | The hero answers WHAT (headline), WHO ("Built for NRIs…"), WHAT YOU HANDLE (supporting line) and WHAT TO DO (Get Started). HOW and WHY TRUST are in the first screen: the command center shows local work, proof and an approval, and the trust strip reads Private by default · Local execution · Visible proof · Customer approval. |

**Branding:** not confirmed. The working name is "NRI Family Office" (`brand.name` in the settings block of src/config/site.ts), with a placeholder mark. See [product-overview.md](product-overview.md).
